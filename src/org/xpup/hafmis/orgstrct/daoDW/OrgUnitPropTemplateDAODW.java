...
public class OrgUnitPropTemplateDAODW extends HibernateDaoSupport {
  ...
  public void checkOupt(OrgUnitPropTemplate orgUnitPropTemplate)
      throws BusinessException {
    Validate.notNull(orgUnitPropTemplate, "参数orgUnitPropTemplate不能为空！");
    Validate.notNull(orgUnitPropTemplate.getName(),
        "orgUnitPropTemplate.getName()不能为空！");
  ...
  public Serializable insertDW(Serializable id,OrgUnitPropTemplate orgUnitPropTemplate)
      throws BusinessException {
    Validate.notNull(orgUnitPropTemplate, "参数orgUnitPropTemplate不能为空！");
    Validate.notNull(orgUnitPropTemplate.getName(),"orgUnitPropTemplate.getName()不能为空！");
    Connection conn=getHibernateTemplate().getSessionFactory().openSession().connection();
    PreparedStatement pre=null;
    String hql="insert into BB103 t (t.id,t.name,t.description,t.inner_name) "
      +" values (?, ?, ?, ?) ";     
    try {
      pre=conn.prepareStatement(hql.toString());
      pre.setString(1, id.toString()); 
      pre.setString(2, orgUnitPropTemplate.getName());    
      pre.setString(3, orgUnitPropTemplate.getDescription());    
      pre.setString(4, orgUnitPropTemplate.getInnerName());     
      pre.executeUpdate(); 
