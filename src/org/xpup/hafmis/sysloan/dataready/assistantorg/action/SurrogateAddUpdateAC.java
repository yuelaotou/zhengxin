/*
 * Generated by MyEclipse Struts
 * Template path: templates/java/JavaClass.vtl
 */
package org.xpup.hafmis.sysloan.dataready.assistantorg.action;

import java.io.IOException;
import java.util.HashMap;
import java.util.Map;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import jcifs.smb.PictureUpload;

import org.apache.commons.beanutils.BeanUtils;
import org.apache.struts.action.ActionForm;
import org.apache.struts.action.ActionForward;
import org.apache.struts.action.ActionMapping;
import org.apache.struts.action.ActionMessage;
import org.apache.struts.action.ActionMessages;
import org.apache.struts.actions.LookupDispatchAction;
import org.xpup.common.util.BSUtils;
import org.xpup.hafmis.common.util.BusiConst;
import org.xpup.hafmis.common.util.BusiTools;
import org.xpup.hafmis.orgstrct.dto.SecurityInfo;
import org.xpup.hafmis.sysloan.common.domain.entity.Developer;
import org.xpup.hafmis.sysloan.dataready.assistantorg.bsinterface.ISurrogateBS;
import org.xpup.hafmis.sysloan.dataready.assistantorg.dto.SurrogateAFDTO;
import org.xpup.hafmis.sysloan.dataready.assistantorg.form.SurrogateAF;
import org.xpup.hafmis.sysloan.dataready.develop.bsinterface.IDevelopBS;
import org.xpup.hafmis.sysloan.dataready.develop.form.DevelopNewAF;

/** 
 * MyEclipse Struts
 * Creation date: 09-28-2007
 * 
 * XDoclet definition:
 * @struts.action validate="true"
 */
public class SurrogateAddUpdateAC extends LookupDispatchAction {
  public static final String PAGINATION_KEY =
    "org.xpup.hafmis.sysloan.dataready.assistantorg.action.SurrogateShowAC";
  protected Map getKeyMethodMap() {
    Map map = new HashMap();
    map.put("button.sure", "sure");
    map.put("button.scan", "scan");
    map.put("button.back", "back");
    return map;
  }
  
  public ActionForward sure(ActionMapping mapping, ActionForm form,
      HttpServletRequest request, HttpServletResponse response) {
    ActionMessages message=null;
    SurrogateAF AF = (SurrogateAF) form;
    SecurityInfo securityInfo = (SecurityInfo) request.getSession()
    .getAttribute("SecurityInfo");
    String error="操作失败!";
    
    SurrogateAFDTO surrogateAFDTO = null;
    
    try{
      ISurrogateBS surrogateBS=(ISurrogateBS)BSUtils
      .getBusinessService("surrogateBS", this, mapping.getModuleConfig());
      if(AF.getType().equals("添加"))
      {
        //张列改 头
        surrogateAFDTO = new SurrogateAFDTO();
        BeanUtils.copyProperties(surrogateAFDTO, AF);
        surrogateBS.insertSurrogateList(surrogateAFDTO,securityInfo);
        //张列改 尾
        
        error="添加成功!";
        //wangshuang   添加
        SurrogateAF surrogateAF=new SurrogateAF();
        Map map = BusiTools.listBusiProperty(BusiConst.DOCUMENTSSTATE);
        surrogateAF.setMap(map);
        surrogateAF.setType("添加");
        Map regionMap = BusiTools.listBusiProperty(BusiConst.INAREA);
        surrogateAF.setRegionMap(regionMap);
        request.setAttribute("AF", surrogateAF);
        return mapping.findForward("to_surrogate_addupdate"); //添加成功
      }
      if(AF.getType().equals("修改"))
      {  
        //张列改 头
        surrogateAFDTO = new SurrogateAFDTO();
        BeanUtils.copyProperties(surrogateAFDTO, AF);
        surrogateBS.updateSurrogate(surrogateAFDTO,securityInfo);
        
        //张列改 尾
        error="修改成功!";
        return mapping.findForward("surrogateShowAC");
      }
    }
    catch(Exception e)
    {
      error="操作失败";
      e.printStackTrace();  
    }finally{
      message= new ActionMessages();
      message.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage(error,false));
      saveErrors(request,message);
    }
    return mapping.findForward("surrogateShowAC"); //添加成功
  }
  
  public ActionForward scan(ActionMapping mapping, ActionForm form,      
      HttpServletRequest request, HttpServletResponse response) {
    PictureUpload pu = new PictureUpload();
    String path;
    SurrogateAF AF = (SurrogateAF) form;
    try
    {
      SurrogateAFDTO surrogateAFDTO = new SurrogateAFDTO();
      SecurityInfo securityInfo = (SecurityInfo) request.getSession().getAttribute("SecurityInfo");
      String serverPath=BusiConst.UPLOAD_SERVER_PATH;
      path = pu.upload(securityInfo.getUserInfo().getUserIp(), "picture", serverPath);
      request.getSession().setAttribute("setIsNeedDel","1");
      BeanUtils.copyProperties(surrogateAFDTO, AF);
      surrogateAFDTO.setPhotoUrl(path);
      ISurrogateBS surrogateBS=(ISurrogateBS)BSUtils
      .getBusinessService("surrogateBS", this, mapping.getModuleConfig());
      surrogateBS.updateSurrogate(surrogateAFDTO, securityInfo);
    }
    catch(IOException e)
    {
        e.printStackTrace();
    }
    catch(Exception ex){
      ex.printStackTrace();
    }
    return mapping.findForward("surrogateShowAC"); //添加成功
  }
  public ActionForward back(ActionMapping mapping, ActionForm form,
      HttpServletRequest request, HttpServletResponse response) {
    return mapping.findForward("surrogateShowAC");
  }
}