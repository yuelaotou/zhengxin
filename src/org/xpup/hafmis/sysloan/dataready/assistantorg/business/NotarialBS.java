  ...
  public List findNotarialList(Pagination pagination)
  {
    ...
    try{
      ...
      if(list.size()!=0||list!=null)
      {
        ...
        {
          NotarialAFDTO aAF=new NotarialAFDTO(); //创建 AF
          NotarialDTO ao=(NotarialDTO)list.get(i);  //DAO 返回实体list
   ...
   */
    public void insertNotarialList(NotarialAFDTO notarialAFDTO,SecurityInfo securityInfo)
    {
      try {
        ...
        aog.setAssistantOrgName(notarialAFDTO.getAssistantOrgName().trim());//担保公司名称
        aog.setArtfclprsn(notarialAFDTO.getArtfclprsn().trim());//法人代表
        aog.setCode(notarialAFDTO.getCode().trim());//组织机构
        aog.setAssistantOrgAddr(notarialAFDTO.getAssistantOrgAddr().trim());//担保公司地址
        aog.setBasedDate(notarialAFDTO.getBasedDate().trim());//成立日期
        aog.setArtfclprsnCardKind(notarialAFDTO.getArtfclprsnCardKind().trim());//法人证件类型
        aog.setArtfclprsnCardNum(notarialAFDTO.getArtfclprsnCardNum().trim());//法人证件号码
        aog.setAllowDept(notarialAFDTO.getAllowDept().trim());//批准机关
        aog.setAllowId(notarialAFDTO.getAllowId().trim());//批准文号
        aog.setAgreementStartDate(notarialAFDTO.getAgreementStartDate().trim());//协议签订日期
        aog.setAgreementEndDate(notarialAFDTO.getAgreementEndDate().trim());//协议到期日期
        aog.setPaybank(notarialAFDTO.getPaybank().trim());//开户银行
        aog.setPayacc(notarialAFDTO.getPayacc().trim());//开户行帐号
        aog.setContactPrsn(notarialAFDTO.getContactPrsn().trim());//联系人
        aog.setContactTel(notarialAFDTO.getContactTel().trim());//联系电话
        aog.setArear(notarialAFDTO.getArear().trim());//所属地区
        aog.setBusiness(notarialAFDTO.getBusiness());//职务
        aog.setRegisterFund(notarialAFDTO.getRegisterFund());//注册资金
        aog.setRemark(notarialAFDTO.getRemark());//备注
        ...
        plOperateLog.setOpModel(String.valueOf(BusiLogConst.PL_OP_DATAPREPARATION_NOTARIZATIONOFFICE));//代理机构维护
    ...
    public void updateNotarial(NotarialAFDTO notarialAFDTO,SecurityInfo securityInfo)
    {
      ...
      try{
        ...
        PlOperateLog plOperateLog=new PlOperateLog();//创建日志对象     
        plOperateLog.setOpSys(new BigDecimal(BusiLogConst.OP_SYSTEM_TYPE_LOAN));//个贷系统
        plOperateLog.setOpModel(String.valueOf(BusiLogConst.PL_OP_DATAPREPARATION_NOTARIZATIONOFFICE));//代理机构
