package org.xpup.hafmis.syscollection.common.biz.orgpop.business;
...
public class OrgpopBS implements IOrgpopBS {
  ...
  /**
   * 根据单位编号、单位名称、单位状态查询单位信息 单位弹出框查询 李娟
   */
  public List findOrgpopList(Pagination pagination, SecurityInfo securityInfo)
    ...
    if (qx.equals("yes")) {
      orgpopList = orgDAO.queryOrgpopListByCriterions(id, name, oldId, orderBy,
          order, start, pageSize, status, securityInfo);
      if (orgpopList != null) {
      ...
      count = orgDAO.queryOrgpopCountByCriterions(id, name, oldId, status,
          securityInfo);
    } else if (qx.equals("no")) {
      orgpopList = orgDAO.queryOrgpopListByCriterionsNo(id, name, oldId,
          orderBy, order, start, pageSize, status, securityInfo);
      if (orgpopList != null) {
        for (int i = 0; i < orgpopList.size(); i++) {
          Org org = (Org) orgpopList.get(i);
          org.getOrgInfo().setTemp_region(
      ...
      count = orgDAO.queryOrgpopCountByCriterionsNo(id, name, oldId, status,
          securityInfo);
    }
