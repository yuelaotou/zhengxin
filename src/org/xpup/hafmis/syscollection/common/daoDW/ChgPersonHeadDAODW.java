...
public class ChgPersonHeadDAODW extends HibernateDaoSupport {
  ...
  public void doClearAccount(String orgStr,String busiDate,String busiDateTime,String docNum,String ip,String op_time,String oper) throws BusinessException, HibernateException, SQLException{
...
//              + criterion.substring(0, criterion.lastIndexOf("and"));
   ...
   * @throws Exception
   */
  public List findAddchgpayListImportList(final String orgid) {
    List list = null;
    try {
      list = (List)getHibernateTemplate().executeFind(new HibernateCallback() {
        public Object doInHibernate(Session session) throws HibernateException,
          ...
          ChgpayHeadImportDTO chgpayHeadImportDTOtemp= new ChgpayHeadImportDTO();
          t.add(chgpayHeadImportDTOtemp);
          Iterator it = queryList.iterator();
          Object obj[] = null;
          while (it.hasNext()) {
            obj = (Object[]) it.next();
            ChgpayHeadImportDTO chgslarybaseHeadImportDTO = new  ChgpayHeadImportDTO();
   ...
   * @throws Exception
   */
  public List findchgpersonImpTitle(final String orgid) {
    ...
    try {
      list = (List)getHibernateTemplate().executeFind(new HibernateCallback() {
        public Object doInHibernate(Session session) throws HibernateException,
            SQLException {
          String sql = "select aa204.org_id, ba001.name, aa204.chng_month"
             + " from AA204 aa204, BA001 ba001,AA001 aa001"
             + " where aa204.org_id = aa001.id"
             + " and aa001.orginfo_id=ba001.id"
             + " and aa204.org_id=? ";           
          Query query = session.createSQLQuery(sql);
          query.setString(0,orgid);
          ...
          while (it.hasNext()) {
            obj = (Object[]) it.next();
            ChgpersonImpTitleDTO chgpersonImpTitleDTO = new  ChgpersonImpTitleDTO();
            if (obj[0] != null && !obj[0].equals(""))
              chgpersonImpTitleDTO.setOrgunitcodecontent(obj[0].toString());
            if (obj[1] != null && !obj[1].equals(""))
              chgpersonImpTitleDTO.setOrgunitnamecontent(obj[1].toString());
            if (obj[2] != null && !obj[2].equals(""))
              chgpersonImpTitleDTO .setOrgunitchgmonthcontent(obj[2].toString());  
             t.add(chgpersonImpTitleDTO);
          }
          ChgpersonImpTitleDTO chgpersonImpTitleDTOtemp = new  ChgpersonImpTitleDTO();
          t.add(chgpersonImpTitleDTOtemp);
          return t;
        }
      });
    } catch (Exception e) {
