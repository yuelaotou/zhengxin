...
public  class ChgPaymentPaymentDAODW extends HibernateDaoSupport{
...
//             + criterion.substring(0, criterion.lastIndexOf("and"));
    ...
    * @return
    */
   public List findAddpaymentListImportList(final String orgheadID) {
     List list = null;
     try {
       list = (List)getHibernateTemplate().executeFind(new HibernateCallback() {
         public Object doInHibernate(Session session) throws HibernateException,
             SQLException {
           String sql = "select emp_id,old_org_pay,old_emp_pay,org_pay,emp_pay,salary_base from aa203 where chng_head_id=?";
             
           Query query = session.createSQLQuery(sql);
           query.setParameter(0,new Integer(orgheadID));
           List queryList = query.list();
           List t = new ArrayList();
           ChgpayListImportDTO tempChgpayListImportDTO= new ChgpayListImportDTO();
           t.add(tempChgpayListImportDTO);
           Iterator it = queryList.iterator();
           Object obj[] = null;
           while (it.hasNext()) {
             obj = (Object[]) it.next();
             ChgpayListImportDTO chgpayListImportDTO = new ChgpayListImportDTO();
             if (obj[0] != null && !obj[0].equals(""))
               chgpayListImportDTO.setEmpId(obj[0].toString());
             if (obj[1] != null && !obj[1].equals(""))
               chgpayListImportDTO.setOldOrgPay(obj[1].toString());
             if (obj[2] != null && !obj[2].equals(""))
               chgpayListImportDTO.setOldEmpPay(obj[2].toString());
             if (obj[3] != null && !obj[3].equals(""))
               chgpayListImportDTO.setOrgPay(obj[3].toString());
             if (obj[4] != null && !obj[4].equals(""))
               chgpayListImportDTO.setEmpPay(obj[4].toString());
             if (obj[4] != null && !obj[5].equals(""))
               chgpayListImportDTO.setSalaryBase(obj[5].toString());
             t.add(chgpayListImportDTO);
           }
           return t;
         }
       });
