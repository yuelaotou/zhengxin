...
public class ChgPaymentTailDAO extends HibernateDaoSupport {
...
//              + criterion.substring(0, criterion.lastIndexOf("and"));
  ...
  public String countOverdueInfoOweMonth(final String contractId)throws Exception{
    ...
    oweMonth = (String)getHibernateTemplate().execute(new HibernateCallback() {
        public Object doInHibernate(Session session) throws HibernateException,
          ...
          String hql = " select max(pl205.owe_month) from pl205 where";
          if(contractId != null && !"".equals(contractId)){
            hql = hql+" pl205.contract_id = '"+contractId+"'";
          }
          Query query = session.createSQLQuery(hql);

          return query.uniqueResult()==null?"0":query.uniqueResult().toString();
        }
      }
      );
    return oweMonth;
  ...
  public List queryByCriterionsAll_wsh(final String id, final String chgMonth,
      final String orderBy, final String order, final int start,
      final int pageSize,final int page) {
    Validate.isTrue(order == null || "ASC".equalsIgnoreCase(order)
        || "DESC".equalsIgnoreCase(order));
    ...
    List orglist = getHibernateTemplate().executeFind(new HibernateCallback() {
      public Object doInHibernate(Session session) throws HibernateException,
        ...
        if (criterion.length() != 0)
          criterion = "where  chgPaymentTail.chgPaymentHead.id=chgPaymentSalaryBase.id and  "
              + criterion.substring(0, criterion.lastIndexOf("and"));
        String ob = orderBy;
        if (ob == null)
          ob = "chgPaymentTail.empId ";
  ...
  public Object[] queryChgPaymentTailPay(final String id) {
    Object[] obj = null;
    try {
      obj = (Object[])getHibernateTemplate().execute(new HibernateCallback() {
        public Object doInHibernate(Session session) throws HibernateException,
        SQLException {
          String sql = "select sum(a3.old_org_pay),sum(a3.old_emp_pay) from aa202 a2,aa203 a3 where a2.id=a3.chng_head_id and a2.id = ? ";
