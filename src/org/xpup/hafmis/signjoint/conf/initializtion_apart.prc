create or replace procedure initializtion_apart(in_v_emplyname tb_emply_mng.emplyname%type, --职工姓名
                                                in_v_emplyacc  tb_emply_mng.emplyacc%type, --职工账号
                                                in_v_emplyid   tb_emply_mng.emplyid%type, --身份证号
                                                in_whichtype   number --查询类型 余额1 明细2 贷款3 贷款明细4
                                                ) is
  ...
  Cursor cr_rq001_empId_orgid is
    select e.emplyacc  zgzh, --职工账号
           e.unitacc   dwzh, --单位账号
           u.unitname  dwmc, --单位名称
           e.emplyname zgxm, --职工姓名
           e.emplyid   sfzh --身份证号
      from tb_emply_mng e, tb_unit_mng u, rq002 r
     where e.unitacc = u.unitacc
       and e.emplyacc = r.empid
       and r.name = in_v_emplyname
       and r.card_num = in_v_emplyid
       and r.empid = in_v_emplyacc;
...
begin
  ...
  loop
    fetch cr_rq001_empId_orgid
      into v_emplyacc, v_unitacc,v_unitname, v_emplyname, v_emplyid;
    ...
    -------------------------------公积金余额开始---------------------------------------------------------
    if in_whichtype = 1 then
    
      delete from lmk_gjjye gjjye_t
       where gjjye_t.orgid = v_unitacc
         and gjjye_t.empid = v_emplyacc; -------------删除公积金余额表中该职工信息
      ...
      select max(s.emplydetail_id)
       ...
       where s.unitacc = v_unitacc
         and s.emplyacc = v_emplyacc
    ...
    ------------------------------------公积金余额结束-----------------------------------------------------
    ------------------------------------公积金明细开始-----------------------------------------------------
    if in_whichtype = 2 then
    
      delete from lmk_gjjmx gjjmx_m
       where gjjmx_m.orgid = v_unitacc
         and gjjmx_m.empid = v_emplyacc; --------------删除公积金明细表中该职工信息
