...
<html>
  ...
  <script type="text/javascript">
	...
	var i=0;
	<%
			TranTbAF tranTbAF=(TranTbAF)request.getSession().getAttribute("tranTbAF_print");
			List list=tranTbAF.getList();
  			TranoutTbDTO dto = null;
  			for(int j=0;j<list.size();j++)
  			{
  				dto = new TranoutTbDTO();
  				dto=(TranoutTbDTO)list.get(j);
 	%>		
				docnum[i]="<%=dto.getDocNum()%>";
				inorgid[i]=formatTen("<%=dto.getOrgInid()%>")+"<%=dto.getOrgInid()%>";
				inorgname[i]="<%=dto.getOrgInName()%>";
				outorgid[i]=formatTen("<%=dto.getOrgOutid()%>")+"<%=dto.getOrgOutid()%>";
				outorgname[i]="<%=dto.getOrgOutName()%>";
				personcount[i]="<%=dto.getCounts()%>";
				summoney[i]="<%=dto.getSumMoney()%>";
				setdate[i]=<%=dto.getSetDate()%>;
				i++;
 	<%
 			}
 	%>				
