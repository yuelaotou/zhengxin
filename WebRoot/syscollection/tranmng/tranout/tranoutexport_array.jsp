...
function goClose() {
   window.opener.document.URL="<%=path%>/syscollection/tranmng/tranout/tranExportTaAC.do?tatol="+document.chgPayExport.elements["tatol"].value+"&empIds="+document.chgPayExport.elements["empIds"].value+"&empNames="+document.chgPayExport.elements["empNames"].value
  +"&cardNums="+document.chgPayExport.elements["cardNums"].value+"&cardKinds="+document.chgPayExport.elements["cardKinds"].value
  +"&outOrgId="+document.chgPayExport.elements["outOrgId"].value+"&outOrgname="+document.chgPayExport.elements["outOrgname"].value
  +"&inOrgId="+document.chgPayExport.elements["inOrgId"].value+"&inOrgName="+document.chgPayExport.elements["inOrgName"].value+"";
	window.close();
}
function goCancel() {
	window.close();
...
function reportErrors() {
	...
	<body bgcolor="#FFFFFF" text="#000000" onload="reportErrors();"
		...
		<form action="<%=path%>/syscollection/tranmng/tranout/tranExportTaAC.do"
			name="chgPayExport" method="POST" style="margin: 0">
			<input type="hidden" name="noteNum" value="<%=noteNum%>" />
			<input type="hidden" name="inOrgId" value="<%=inOrgId%>" />
			<input type="hidden" name="outOrgId" value="<%=outOrgId%>" />
			<input type="hidden" name="outOrgname" value="<%=outOrgname%>" />
			<input type="hidden" name="inOrgName" value="<%=inOrgName%>" />
