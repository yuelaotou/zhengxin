...
<script src="<%=path%>/js/common.js">
  ...
  <script type="text/javascript">
	...
	<%
 	...
 	%>
		...
		if(<%=tranTbPrintAF.getNote_num()==null %>){
			...
			noteNum = "<%=tranTbPrintAF.getDoc_num() %>";
		}
	...
	%>
			...
			date_yg="<%=tail.getEmp().getOrgPayMonth()%>";
			balance="<%=tail.getEmp().getPreBalance().add(tail.getEmp().getCurBalance())%> ";
...
<%--				 --%>
<%--				document.forms(0).Cell1.insertSheetFromFile("<%=path%>/syscollection/tranmng/report/tranout_yg.cll",0,1,1);--%>
...
<%--			    document.forms(0).Cell1.PrintSetMargin(180,200,180,320);--%>
		...
		if(count!=-1){//计算小数后面
			...
			document.forms(0).Cell1.S(20,8,0,fin);
		     ...
		     document.forms(0).Cell1.S(12,4,0,empId[0]);
		     ...
		     document.forms(0).Cell1.S(5,4,0,tranin_empid);
		     document.forms(0).Cell1.S(4,6,0,empName[0]);
		     document.forms(0).Cell1.S(7,6,0,cardNum[0]);
		     document.forms(0).Cell1.S(16,6,0,tranreason);
