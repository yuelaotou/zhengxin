...
<%@ page import="org.xpup.hafmis.syscollection.tranmng.tranin.form.TranTbPrintAF" %>
  ...
  <script type="text/javascript">
	...
	<%
 	...
 	%>	
		...
		%>
				traninempid[b]="<%=tail.getEmpId() %>";
				tranoutempid[b]="<%=tail.getMergerEmpid() %>";
				empName[b]="<%=tail.getName() %> ";
				cardNum[b]="<%=tail.getCardNum() %>";
				tranoutsum[b]="<%=tail.getSumBalance() %>";
				sumMoney[b]="<%=tail.getSumMoney() %>";
				curIntegral[b]="<%=tail.getCurIntegral() %>";
				salaryBase[b]="<%=tail.getSalaryBase() %>";
