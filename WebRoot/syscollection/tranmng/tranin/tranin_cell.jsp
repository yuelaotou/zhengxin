...
<script src="<%=path%>/js/common.js">
  ...
  <script type="text/javascript">
	...
	%>
 	...
 	%>
 	            integral[i]="<%=biga%>";
 	            if(<%=tranOutTails.getEmpId()==null || tranOutTails.getEmpId().intValue()==0 %>){
 	            	empid[i]="";
 	            }else{
					empid[i]=format("<%=tranOutTails.getEmpId() %>")+"<%=tranOutTails.getEmpId() %>";
				}
				if(<%=tranOutTails.getTranin_empid()==null || tranOutTails.getTranin_empid().intValue()==0 %>){
					inempid[i]="";
				}else{
					inempid[i]=format("<%=tranOutTails.getTranin_empid() %>")+"<%=tranOutTails.getTranin_empid() %>";
				}
