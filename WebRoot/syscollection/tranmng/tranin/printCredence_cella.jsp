  ...
  <script type="text/javascript">
	...
	<% 
		...
		for(int j =0;j<list.size();j++){
			TranInTail tail = (TranInTail)list.get(j);
	...
	%>
			integral="<%=biga%>";
			salary="<%=tail.getMonthIncome() %> ";
			if(<%=tail.getEmpId()==null || tail.getEmpId().toString().equals("0") %>){
				empId[i]="";
			}else{
				empId[i]=format("<%=tail.getEmpId() %> ")+"<%=tail.getEmpId() %> ";
			}
			empName[i]="<%=tail.getName() %> ";
 	...
 	%>   
				...
				if(<%=size==1%>){
					...
					document.forms(0).Cell1.S(7,12,0,tranoutsum[0]);//发生额
