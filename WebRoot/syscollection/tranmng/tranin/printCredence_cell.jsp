  ...
  <script type="text/javascript">
	...
	%>
			...
			if(<%=tail.getTranin_empid().intValue()==0%>){
				...
				tranin_empid[i]=format("<%=tail.getTranin_empid()%>")+"<%=tail.getTranin_empid()%>";
			}
			...
			if(<%=tail.getEmp().getEmpId()==null || tail.getEmp().getEmpId().toString().equals("0") %>){
				empId[i]="";
			}else{
 	...
 	%>   
		...
		var sumMoney=0;
				...
				document.forms(0).Cell1.S(3,9,0,traninbankname);
				...
				document.forms(0).Cell1.S(9,9,0,tranoutbankname);
				
				if(<%=size==1%>){
					document.forms(0).Cell1.S(1,12,0,empId[0]);//转出个人账号
					...
					document.forms(0).Cell1.S(16,12,0,tranin_empid[0]);//转入个人账号
				}
