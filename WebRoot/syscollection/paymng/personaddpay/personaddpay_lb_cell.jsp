...
<html>
  ...
  <script type="text/javascript">
	...
	loginReg();
	document.forms(0).Cell1.openfile("<%=path%>/syscollection/paymng/report/personaddpay_maintain_cell.cll","");
 	...
 	%>				
		...
		for(k = 0 ; k < totalLine; k++){
			if(k%totalPageLine==0&&k>0)
			{
				...
				pageCurrent++;	
				completeline=k-2;				
				document.forms(0).Cell1.insertSheetFromFile("<%=path%>/syscollection/paymng/report/personaddpay_maintain_cell.cll",0,1,pageCurrent);
			...
			if(pageCurrent==0)
			{
				...
				document.forms(0).Cell1.S(5,2,0,userName);
				...
				document.forms(0).Cell1.s(7,k+4,0,paydate[k]);
				...
				document.forms(0).Cell1.s(7,k-completeline+2,pageCurrent,paydate[k]);
				paycounts=paycounts+parseFloat(paycount[k]);
				paymoneys=paymoneys+parseFloat(paymoney[k]);
			}	
		}
