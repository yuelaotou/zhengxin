...
<html>
  ...
  <script type="text/javascript">
	...
	loginReg();
	document.forms(0).Cell1.openfile("<%=path%>/syscollection/paymng/report/orgaddpay_lb_cell.cll","");
	...
	<%
    		List list=(List) request.getAttribute("cellList");
  			OrgaddpayMaintainDto dto=null;
  			for(int j=0;j<list.size();j++)
  			{
  				dto=(OrgaddpayMaintainDto)list.get(j);							
 	%>		
 	...
 	%>				
		...
		for(k = 0 ; k < totalLine; k++){
			if(k%totalPageLine==0&&k>0)
			{
				...
				pageCurrent++;	
				completeline=k-2;				
				document.forms(0).Cell1.insertSheetFromFile("<%=path%>/syscollection/paymng/report/orgaddpay_lb_cell.cll",0,1,pageCurrent);
