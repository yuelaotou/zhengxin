...
<%@ page import="org.xpup.hafmis.syscollection.paymng.orgaddpay.dto.*" %>
...
<html>
  ...
  <script type="text/javascript">
	function load(){	
	loginReg();
	document.forms(0).Cell1.openfile("<%=path%>/syscollection/paymng/report/orgaddpay.cll","");

	<%
    		OrgaddpayHeadPrintDto dto=(OrgaddpayHeadPrintDto)request.getAttribute("orgaddpay");
	%>
		...
		document.forms(0).Cell1.S(3,7,0,"<%=dto.getBankid()%>");
		document.forms(0).Cell1.S(3,6,0,"<%=dto.getBankname()%>");
		document.forms(0).Cell1.S(8,5,0,"<%=dto.getOffice()%>");
		document.forms(0).Cell1.S(8,6,0,"<%=dto.getCollbankname()%>");
		...
		document.forms(0).Cell1.S(1,11,0,"<%=dto.getStartPayMonth()%>"+"-"+"<%=dto.getPayMonth()%>");
