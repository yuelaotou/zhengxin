...
<%@ page import="org.xpup.hafmis.syscollection.querystatistics.paymentpickstatistics.pickyearreport.dto.PickYearRepInfoDTO" %>
<%@ page import="org.xpup.hafmis.syscollection.querystatistics.paymentpickstatistics.pickyearreport.action.PickYearReportShowAC" %>
...
<%
   ...
   Pagination pagination = (Pagination) request.getSession().getAttribute(PickYearReportShowAC.PAGINATION_KEY);
...
<script type="text/javascript">
	...
	loginReg();
	//加载模板文件..
	document.forms(0).Cell1.openfile("<%=path%>/syscollection/querystatistics/paymentpickstatistics/pickyearreport/report/pickyearreport.cll","");
	var month=[];
	//购房
	...
	<%
		...
		PickYearRepInfoDTO dto = null;
		for(int j=0;j<list.size();j++){
			dto = (PickYearRepInfoDTO)list.get(j);
	%>
			month[i]="<%=dto.getMonth()%>";
	...
	%>
		...
		for(k = 0 ; k < totalLine; k++){
			...
			if(k%totalPageLine==0&&k>0)
			{
				...
				pageCurrent++;	
				completeline=k-2;				
				document.forms(0).Cell1.insertSheetFromFile("<%=path%>/syscollection/querystatistics/paymentpickstatistics/pickyearreport/report/pickyearreport.cll",0,1,pageCurrent);
				document.forms(0).Cell1.setSheetLabel(pageCurrent,"第"+(pageCurrent+1)+"页")	;
	
			}
			document.forms(0).Cell1.s(8,3,0,"<%=year%>");
			document.forms(0).Cell1.s(9,19,0,"<%=date%>");
			document.forms(0).Cell1.s(21,19,0,"<%=operator%>");
