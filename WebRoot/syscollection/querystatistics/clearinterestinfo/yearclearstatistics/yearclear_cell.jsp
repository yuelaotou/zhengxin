...
<html>
  ...
  <script type="text/javascript">
	...
	<%
    		YearclearstatisticsHeadDTO f=(YearclearstatisticsHeadDTO)request.getAttribute("YearclearstatisticsHeadDTO");
    		String bankname=(String)request.getAttribute("jizhangren");
    		String office=(String)request.getAttribute("fuheren");
	...
	%>	
		...
		//贷
		document.forms(0).Cell1.S(3,3,0,"<%=setDate.substring(0,4)%>年"+"<%=setDate.substring(4,6)%>月"+"<%=setDate.substring(6,8)%>日");
		document.forms(0).Cell1.S(5,9,0,"<%=userName%>");
		document.forms(0).Cell1.S(2,5,0,"<%=f.getOrgname() %>");
		document.forms(0).Cell1.S(5,5,0,formatTen("<%=f.getOrgcode() %>")+"<%=f.getOrgcode() %>");
		document.forms(0).Cell1.d(2,6,0,"<%=f.getOldcurblance() %>");
		document.forms(0).Cell1.d(2,7,0,"<%=f.getPreinterest() %>");
		document.forms(0).Cell1.S(5,6,0,"<%=f.getEmpcounts() %>");
		document.forms(0).Cell1.S(2,8,0,"<%=f.getBank() %>");
		document.forms(0).Cell1.d(5,7,0,"<%=f.getBlance() %>");		
		document.forms(0).Cell1.d(5,8,0,"<%=f.getGuazhang() %>");	
		document.forms(0).Cell1.s(2,9,0,"<%=f.getSet_headid() %>");			
