	...
	function load(){
	    ...
	    for(int j=0;j<continuumPrintList.size();j++){
 		...
 		%>		
		  ...
		  //	credenceDate[i]="<%=credencePopInfoDTO.getCredenceDate()%>";
		...
		var pageCurrent=0;						//当前页
 		...
 		for(k = 0 ; k < totalLine; k++){
            ...
            document.forms(0).Cell1.S(1,3,pageCurrent,"附原始凭证"+oldCredenceNum[k]+"张");
			document.forms(0).Cell1.S(4,3,pageCurrent,credenceDate[k]+"年"+credenceDate2[k]+"月"+credenceDate3[k]+"日");
			document.forms(0).Cell1.S(9,3,pageCurrent,credenceNum[k]+"-"+(temp_page)+"/"+iPage);	
			...
			document.forms(0).Cell1.d(7,13,pageCurrent,sumDebit[k]);
			document.forms(0).Cell1.d(9,13,pageCurrent,sumCredit[k]);
			
			
			document.forms(0).Cell1.S(4,14,pageCurrent,checkpsn[k]);
			//document.forms(0).Cell1.S(8,14,pageCurrent,makebill[k]);

            document.forms(0).Cell1.S(6,14,pageCurrent,"制单人:"+makebill[k]);
			document.forms(0).Cell1.S(7,14,pageCurrent,"复核:"+checkpsn[k]);
			document.forms(0).Cell1.S(9,14,pageCurrent,"出纳:"+accountpsn[k]);
			...
			for(l = 0 ; l < totalLine1; l++){
				...
				if(l<=totalPageLine-1){
					...
					document.forms(0).Cell1.S(5,l+5,pageCurrent,subjectCodeArray[k][l]);
					document.forms(0).Cell1.d(7,l+5,pageCurrent,debitArray[k][l]);
					document.forms(0).Cell1.d(9,l+5,pageCurrent,creditArray[k][l]);
				}else{//向第一页后的所有页插数据
					...
					document.forms(0).Cell1.S(5,l-completeline+3,pageCurrent,subjectCodeArray[k][l]);
					document.forms(0).Cell1.d(7,l-completeline+3,pageCurrent,debitArray[k][l]);
					document.forms(0).Cell1.d(9,l-completeline+3,pageCurrent,creditArray[k][l]);
				}	
			}
 		}
