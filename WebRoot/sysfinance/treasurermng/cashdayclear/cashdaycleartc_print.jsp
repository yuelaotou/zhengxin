...
<%@ page import="org.xpup.hafmis.sysfinance.treasurermng.cashdayclear.dto.CashDayClearTcShowListDTO" %>
...
<script type="text/javascript">
	...
	loginReg();
	<%if(type.equals("0")){%>	
		document.forms(0).Cell1.openfile("<%=path%>/sysfinance/treasurermng/report/cashdaycleartc.cll","");
  	<%}%>
  	<%if(type.equals("1")){%>	
		document.forms(0).Cell1.openfile("<%=path%>/sysfinance/treasurermng/report/bankdaycleartc.cll","");
  	<%}%>
  	var credenceDate=[];
  	var credenceChaNum=[];
    var summary=[];
	...
	<%
		...
		cashDayClearTcShowListDTO = (CashDayClearTcShowListDTO)list.get(j);
	%>
	    //把数据传到JS的数组里面..
		credenceDate[i] = "<%=cashDayClearTcShowListDTO.getCredenceDate()%>"; 
		credenceChaNum[i] = "<%=cashDayClearTcShowListDTO.getCredenceChaNum()%>";
		summary[i] = "<%=cashDayClearTcShowListDTO.getTemp_summary()%>";
		subjectCode[i] = "<%=cashDayClearTcShowListDTO.getSubjectCode()%>";
		subjectName[i] = "<%=cashDayClearTcShowListDTO.getSubjectName()%>"; 
		debit[i] = "<%=cashDayClearTcShowListDTO.getDebit()%>";
		credit[i] = "<%=cashDayClearTcShowListDTO.getCredit()%>";
		settNum[i] = "<%=cashDayClearTcShowListDTO.getSettNum()%>";
		settType[i] = "<%=cashDayClearTcShowListDTO.getTemp_settType()%>";
		dopsn[i] = "<%=cashDayClearTcShowListDTO.getDopsn()%>";
		makebill[i] = "<%=cashDayClearTcShowListDTO.getMakebill()%>";
		credenceSt[i] = "<%=cashDayClearTcShowListDTO.getCredenceSt()%>";
		i++;
	<%}%>
	...
	for(k = 0 ; k < totalLine; k++){
		if(k%totalPageLine==0&&k>0)
		{
			...
			<%if(type.equals("0")){%>	
				document.forms(0).Cell1.insertSheetFromFile("<%=path%>/sysfinance/treasurermng/report/cashdaycleartc.cll",0,1,pageCurrent);
  			<%}%>
  			<%if(type.equals("1")){%>	
				document.forms(0).Cell1.insertSheetFromFile("<%=path%>/sysfinance/treasurermng/report/bankdaycleartc_1.cll",0,1,pageCurrent);
  			<%}%>					
		...
		if(pageCurrent==0)
		{	
			document.forms(0).Cell1.S(1,k+4,0,credenceDate[k]);
			document.forms(0).Cell1.S(2,k+4,0,credenceChaNum[k]);
			document.forms(0).Cell1.S(3,k+4,0,summary[k]);
			document.forms(0).Cell1.S(4,k+4,0,subjectCode[k]);
			document.forms(0).Cell1.S(5,k+4,0,subjectName[k]);
			document.forms(0).Cell1.d(6,k+4,0,debit[k]);
			document.forms(0).Cell1.d(7,k+4,0,credit[k]);
			document.forms(0).Cell1.S(8,k+4,0,settType[k]);
			document.forms(0).Cell1.S(9,k+4,0,settNum[k]);
			document.forms(0).Cell1.S(10,k+4,0,dopsn[k]);
			document.forms(0).Cell1.S(11,k+4,0,makebill[k]);
			document.forms(0).Cell1.S(12,k+4,0,credenceSt[k]);
			...
			document.forms(0).Cell1.S(2,k-completeline+2,pageCurrent,credenceChaNum[k]);
			document.forms(0).Cell1.S(3,k-completeline+2,pageCurrent,summary[k]);
			document.forms(0).Cell1.S(4,k-completeline+2,pageCurrent,subjectCode[k]);
			document.forms(0).Cell1.S(5,k-completeline+2,pageCurrent,subjectName[k]);
			document.forms(0).Cell1.d(6,k-completeline+2,pageCurrent,debit[k]);
			document.forms(0).Cell1.S(7,k-completeline+2,pageCurrent,credit[k]);
			document.forms(0).Cell1.S(8,k-completeline+2,pageCurrent,settType[k]);
			document.forms(0).Cell1.S(9,k-completeline+2,pageCurrent,settNum[k]);
			document.forms(0).Cell1.S(10,k-completeline+2,pageCurrent,dopsn[k]);
			document.forms(0).Cell1.S(11,k-completeline+2,pageCurrent,makebill[k]);
			document.forms(0).Cell1.S(12,k-completeline+2,pageCurrent,credenceSt[k]);
			debittotal=debittotal+parseFloat(debit[k]);
			credittotal=credittotal+parseFloat(credit[k]);
		}		
	}
