...
<%@ page import="org.xpup.hafmis.sysfinance.systemmaintain.queryoperationlog.dto.QueryOperationLogListDTO" %>
...
<script type="text/javascript">
	function load(){
	loginReg();
	document.forms(0).Cell1.openfile("<%=path%>/sysfinance/systemmaintain/queryoperationlog/report/queryoperationlog.cll","");
	var bizId=[];
	var bizType=[];
	var action=[];
	var opTime=[];
	...
	<%
    		List list=(List) request.getSession().getAttribute("printList");
  			...
  			{
  				queryOperationLogListDTO=(QueryOperationLogListDTO)list.get(j);
 	%>	
				bizId[i]="<%=queryOperationLogListDTO.getBizId()%>";
				bizType[i]="<%=queryOperationLogListDTO.getBizType()%>";
				action[i]="<%=queryOperationLogListDTO.getAction()%>";
				opTime[i]="<%=queryOperationLogListDTO.getOpTime()%>";
				operator[i]="<%=queryOperationLogListDTO.getOperator()%>";
				i++;
 	<%
 			}
 	%>
		...
		for(k = 0 ; k < totalLine; k++){
			if(k%totalPageLine==0&&k>0)
			{
				...
				pageCurrent++;	
				completeline=k-2;				
				document.forms(0).Cell1.insertSheetFromFile("<%=path%>/sysfinance/systemmaintain/queryoperationlog/report/queryoperationlog.cll",0,1,pageCurrent);
				document.forms(0).Cell1.setSheetLabel(pageCurrent,"第"+(pageCurrent+1)+"页")	
			}
			if(pageCurrent==0)
			{
				document.forms(0).Cell1.S(1,k+4,0,bizId[k]);
				document.forms(0).Cell1.S(3,k+4,0,bizType[k]);
				document.forms(0).Cell1.S(5,k+4,0,action[k]);
				document.forms(0).Cell1.S(7,k+4,0,opTime[k]);
				document.forms(0).Cell1.S(9,k+4,0,operator[k]);
			}else{//向第一页后的所有页插数据
				document.forms(0).Cell1.S(1,k-completeline+2,pageCurrent,bizId[k]);
				document.forms(0).Cell1.S(3,k-completeline+2,pageCurrent,bizType[k]);
				document.forms(0).Cell1.S(5,k-completeline+2,pageCurrent,action[k]);
				document.forms(0).Cell1.S(7,k-completeline+2,pageCurrent,opTime[k]);
