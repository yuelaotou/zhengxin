...
<script type="text/javascript">
	...
	loginReg();
	<%if(type.equals("0")){%>	
		document.forms(0).Cell1.openfile("<%=path%>/sysfinance/accmng/report/cashdayclearreport.cll","");
  	<%}%>
  	<%if(type.equals("1")){%>	
		document.forms(0).Cell1.openfile("<%=path%>/sysfinance/accmng/report/bankdayclearreport.cll","");
  	<%}%>
  	var credenceDate=[];
  	var credenceChaNum=[];
    var summary=[];
	...
	%>
		...
		settDate[i] = "<%=cashDayClearTcShowListDTO.getSettDate()%>";
		i++;
	<%}%>
	...
	for(k = 0 ; k < totalLine; k++){
		if(k%totalPageLine==0&&k>0)
		{
			...
			<%if(type.equals("0")){%>	
				document.forms(0).Cell1.insertSheetFromFile("<%=path%>/sysfinance/accmng/report/cashdayclearreport.cll",0,1,pageCurrent);
  			<%}%>
  			<%if(type.equals("1")){%>	
				document.forms(0).Cell1.insertSheetFromFile("<%=path%>/sysfinance/accmng/report/bankdayclearreport.cll",0,1,pageCurrent);
  			<%}%>					
		...
		{	
			...
			document.forms(0).Cell1.S(9,k+4,0,settDate[k]);
			...
			document.forms(0).Cell1.S(9,k-completeline+2,pageCurrent,settDate[k]);
			debittotal=debittotal+parseFloat(debit[k]);
			credittotal=credittotal+parseFloat(credit[k]);
		}		
	}
