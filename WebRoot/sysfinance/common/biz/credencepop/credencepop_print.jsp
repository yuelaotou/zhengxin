...
<html>
  ...
  function load(){
  		loginReg();
	  	document.forms(0).Cell1.openfile("<%=path%>/sysfinance/common/biz/report/credencepop_print7.cll","");
	    ...
	    for(int j=0;j<list.size();j++){
			...
			sumDebit = sumDebit.add(dto.getDebit().divide(new BigDecimal(100),2,BigDecimal.ROUND_UP));
			sumCredit = sumCredit.add(dto.getCredit().divide(new BigDecimal(100),2,BigDecimal.ROUND_UP));
			
 		%>
 			summay[i]="<%=dto.getSummay()%>";
 			freeSummay[i]="<%=dto.getFreeSummay()%>";
 			settnum_t[i]="<%=dto.getSettnum()%>";
 			subjectCode[i]="<%=dto.getSubjectCode()%>"+"-"+"<%=dto.getSubjectName()%>";
 			debit[i]="<%=dto.getDebit().divide(new BigDecimal(100), 2,
								BigDecimal.ROUND_UP)%>";
 			credit[i]="<%=dto.getCredit().divide(new BigDecimal(100), 2,
								BigDecimal.ROUND_UP)%>";
 			i++;
 		<%
 			}
		...
		for(k = 0 ; k < totalLine; k++){
			if(k%totalPageLine==0&&k>0){
				...
				pageCurrent++;	
				completeline=k-2;				
				document.forms(0).Cell1.insertSheetFromFile("<%=path%>/sysfinance/common/biz/report/credencepop_print7.cll",0,1,pageCurrent);
			...
			if(pageCurrent==0){
				...
				document.forms(0).Cell1.S(9,3,pageCurrent,credenceNum+"-"+(pageCurrent+1)+"/"+iPage);	
				
				document.forms(0).Cell1.S(1,k+5,pageCurrent,summay[k]);
				document.forms(0).Cell1.S(3,k+5,pageCurrent,settnum_t[k]);
				...
				document.forms(0).Cell1.d(7,13,pageCurrent,sumDebit);
				document.forms(0).Cell1.d(9,13,pageCurrent,sumCredit);
				
				
				document.forms(0).Cell1.S(4,14,pageCurrent,checkpsn);
				document.forms(0).Cell1.S(6,14,pageCurrent,"制单人:"+makebill);
				document.forms(0).Cell1.S(7,14,pageCurrent,"复核:"+checkpsn);
				document.forms(0).Cell1.S(9,14,pageCurrent,"出纳:"+accountpsn);
			  ...
			  //  document.forms(0).Cell1.S(1,1,pageCurrent,paramExplain+"凭证");
				...
				document.forms(0).Cell1.S(9,3,pageCurrent,credenceNum+"-"+(pageCurrent+1)+"/"+iPage);	
               	...
               	document.forms(0).Cell1.S(1,k-completeline+3,pageCurrent,summay[k]);
				document.forms(0).Cell1.S(3,k-completeline+3,pageCurrent,settnum_t[k]);
				document.forms(0).Cell1.S(5,k-completeline+3,pageCurrent,subjectCode[k]);
				document.forms(0).Cell1.d(7,k-completeline+3,pageCurrent,debit[k]);
				document.forms(0).Cell1.d(9,k-completeline+3,pageCurrent,credit[k]);
				...
				document.forms(0).Cell1.S(4,14,pageCurrent,checkpsn);
			//	document.forms(0).Cell1.S(8,14,pageCurrent,makebill);
				document.forms(0).Cell1.S(6,14,pageCurrent,"制单人:"+makebill);
				document.forms(0).Cell1.S(7,14,pageCurrent,"复核:"+checkpsn);
				document.forms(0).Cell1.S(9,14,pageCurrent,"出纳:"+accountpsn);
			}	
