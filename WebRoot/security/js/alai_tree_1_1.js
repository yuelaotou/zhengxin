...
修订时间：2005-08-20
修订内容：
1、node的label和icon鼠标右击事件重定义
变更类型：不符合要求的行为修正
变更描述：将原来node的label和icon的鼠标右击事件的不选中处理方式修正为选中但不执行脚本事件。
变更内容：
--node的label和icon的onmousedown事件修改为onmousedown=function(){run(node.select);}
2、让目录树支持根据itemId获得node的能力
变更类型：添加新特性
变更描述：让目录树支持根据itemId获得node的能力
变更内容：
--为node添加一个itemId属性，用以唯一标识一个菜单。
--在addNode、node.add、node.addLink、tree.add等方法中加入一个参数itemId。
--为tree添加了一个getNodeById方法，用以通过itemId查找菜单项node。
3、错误图标显示的修正
变更类型：bug修正
变更描述：当某一菜单项的所有子菜单都被删除后，其图标应变回叶子节点时的图标。原程序没有，而是没有变换图标。
变更内容：
--为tree增加一个afterremove事件。
--在tree的removeNode的方法最后加入run(tree.afterremove,srcNode)。
--在alai_tree_help.js文件中增加afterremove事件处理，并在该事件中，在适当的时候将图标更换。
4、当所有菜单都被删除后无法新建菜单问题的修正
变更类型:bug修正
变更描述：
变更内容：
--在addNode的switch(relation)语句的default分支处加入if(toNode.last!=null)。其bug产生原因是在没判断toNode.last
是否为空时，就直接使用toNode.last.next。
5、添加node.getSiblingIndex方法
********************************************************************************/
...
{try{
	...
	var addNode=function(toNode,relation,itemId,text,key,ico,exeCategory,exeArg)
	{
		var node={}
		...
		{	case "first":
			...
			default:
				...
				node.parent=toNode
				if(toNode.hasChild){node.prev=toNode.last;if(toNode.last!=null)toNode.last.next=node;toNode.last=node;}
				else{toNode.first=toNode.last=node;}
		}
		...
		with(label)
		{
			...
			onmousedown=function(){run(node.select);}
		...
		node.label=label
		node.icon=icon
		node.add=function(itemId,text,ico,exeType,exeArg){return addNode(this,"last",itemId,text,"",ico,exeType,exeArg);}
		...
		node.getSibling=function(){return node.parent.children};
                node.getSiblingIndex=function(){var cc=node.parent.children.length;for(var i=0;i<cc;i++)if(node.parent.children[i]==node)return i+1;return -1;};
	...
	this.removeNode=function(srcNode){if(!srcNode.hasChild)remove(srcNode);else{for(var i=0;i<srcNode.children.length;i++){tree.removeNode(srcNode.children[i]);}tree.removeNode(srcNode);}run(tree.afterremove,srcNode)}
	root.add=function(itemId,text,ico,exeType,exeArg){return addNode(root,"last",itemId,text,"",ico,exeType,exeArg);}
	...
	this.onclick=this.ondblclick=this.onmousemove=this.onmouseover=this.onmouseout=this.onmouseup=this.onmousedown=this.onkeypress=this.onkeydown=this.onkeyup=this.oncollapse=this.onexpand=this.afteradd=this.afterremove=this.onblur=this.onfocus=this.onselect=new Function("return true;");
	this.add=addNode
	this.getNodeById=function(itemId){if(itemId==null||itemId=="")return null;for(var i=count-1;i>=0;i--)if(nodes[i].itemId==itemId)return nodes[i];return null;}
