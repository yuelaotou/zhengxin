...
try{
	...
	{	
		...
		with(nbody)
		{	insertAdjacentElement("beforeEnd",label1)
		...
		if(toNode.getTier()==0){
			...
			label1.innerHTML="<input type='checkBox' onclick='checks"+count_var+"(this,"+'"'+count_var+'"'+")'>";
		}
		...
		if(showExpand & !showBranch){
		icon.style.marginLeft=icons["expand"].width}
		node.label=label
		node.icon=icon
