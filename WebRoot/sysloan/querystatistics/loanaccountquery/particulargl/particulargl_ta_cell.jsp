  ...
  var i=0;
	...
	%>
 	...
 	%>
		     ...
		     document.forms(0).Cell1.d(4,7,0,"<%=particularglTaAF.getOveaerloanrepay() %>");
		     ...
		     document.forms(0).Cell1.S(2,13,0,"<%=opertname %>");
		      ...
		      document.forms(0).Cell1.S(2,8,0,"<%=particularglTaAF.getOverlimited()%>");
		      document.forms(0).Cell1.d(2,12,0,"<%=particularglTaAF.getCorputInterest()%>");
		      document.forms(0).Cell1.S(4,8,0,"<%=particularglTaAF.getOtherArrearage()%>");
		      ...
		      document.forms(0).Cell1.d(4,12,0,"<%=particularglTaAF.getShouldCorputInterest()%>");
		...
		for(k = 0 ; k < totalLine; k++){
			...
			{      
				...
				document.forms(0).Cell1.PrintSetCustomPaper(220,280,pageCurrent);
    			document.forms(0).Cell1.PrintSetMargin(15,10,15,10);
			}
			if(pageCurrent==1)
			{		document.forms(0).Cell1.S(1,2,1,"操作员：<%=opertname%>");
				document.forms(0).Cell1.S(6,2,1,"操作时间：<%=time%>");
			...
			document.forms(0).Cell1.S(1,2,1,"操作员：<%=opertname%>");
				...
				if(completeline==0)//查询出来的记录可以在一页显示的时候会进入
				{
					...
					document.forms(0).Cell1.DeleteRow(totalLine+6,35-totalLine-5,pageCurrent);
				...
				else
				{
					...
					//document.forms(0).Cell1.S(10,totalLine-completeline+3,pageCurrent,"<%=time %>");
					 ...
					 document.forms(0).Cell1.DeleteRow(totalLine-completeline+4,35-(totalLine-completeline+3),pageCurrent);
					document.forms(0).Cell1.ReDraw();
				}	
...
<td><INPUT id="Button3" onclick="location.href='particularglTaShowAC.do'" type="button" value=" 返回 "></td>	
</table>
</form>
  </body>
</html>
