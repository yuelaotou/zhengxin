...
<html>
	...
	function load(){
	loginReg();	
	document.forms(0).Cell1.openfile("<%=path%>/sysloan/specialbiz/report/bailenroltb.cll","");
	var contractId=[];
	var borrowerName=[];
	var cardNum=[];
	var loanBankName=[];
	...
	<%
		...
		bailenRolTbDTO = (BailenRolTbDTO)list.get(j);
	%>
	    //把数据传到JS的数组里面..
		contractId[i] = "<%=bailenRolTbDTO.getContractId()%>"; 
		borrowerName[i] = "<%=bailenRolTbDTO.getBorrowerName()%>";
		cardNum[i] = "<%=bailenRolTbDTO.getCardNum()%>";
		loanBankName[i] = "<%=bailenRolTbDTO.getLoanBankName()%>";
		bizDate[i] = "<%=bailenRolTbDTO.getBizDate()%>";
		occurMoney[i] = "<%=bailenRolTbDTO.getOccurMoney()%>";
		bizSt[i] = "<%=bailenRolTbDTO.getBizSt()%>";
		i++;
	<%}%>
	...
	var occurMoneytotal=0;					//保证金登记发生额合计
	for(k = 0 ; k < totalLine; k++){
		if(k%totalPageLine==0&&k>0)
		{
			...
			document.forms(0).Cell1.insertSheetFromFile("/hafmis/sysloan/specialbiz/report/bailenroltb.cll",0,1,pageCurrent);
		...
		{	
			...
			document.forms(0).Cell1.S(4,k+4,0,loanBankName[k]);
			document.forms(0).Cell1.S(5,k+4,0,bizDate[k]);
			document.forms(0).Cell1.d(6,k+4,0,occurMoney[k]);
			document.forms(0).Cell1.S(7,k+4,0,bizSt[k]);
			...
			document.forms(0).Cell1.S(5,k-completeline+2,pageCurrent,bizDate[k]);
			document.forms(0).Cell1.d(6,k-completeline+2,pageCurrent,occurMoney[k]);
			document.forms(0).Cell1.S(7,k-completeline+2,pageCurrent,bizSt[k]);
			occurMoneytotal=occurMoneytotal+parseFloat(occurMoney[k]);
		}		
	}
