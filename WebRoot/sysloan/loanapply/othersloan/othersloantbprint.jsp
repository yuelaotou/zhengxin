...
document.forms(0).Cell1.Login("沈阳金软科技有限公司","","13100104509","0740-1662-7775-3003"); 
	document.forms(0).Cell1.openfile("<%=path%>/sysloan/loanapply/othersloan/othersLoanprint.cll","");
	var id=[];
	var name=[];
	var empId=[];
	var orgName=[];
	...
	<%
 	...
 	%>		
 				id[i]="<%=othersLoan.getId()%>";
 				name[i]="<%=othersLoan.getBorrowerName()%>";
				empId[i]="<%=othersLoan.getBorrowerEmpId()%>";
				orgName[i]="<%=othersLoan.getBORROWERORGNAME()%>";
				orgId[i]="<%=othersLoan.getBORROWERORGID()%>";
				...
				loanCity[i]="<%=othersLoan.getLOANCITY()%>"
				beiYong[i]="<%=othersLoan.getBORROWERADDR()%>"
				i++;
 	<%
 			}
 	%>
		...
		for(k = 0 ; k < totalLine; k++){
			...
			{//插入新页.对上一页求和的完成
				...
				document.forms(0).Cell1.insertSheetFromFile("<%=path%>/sysloan/loanapply/othersloan/othersLoanprint.cll",0,1,pageCurrent);
			...
			{		
				...
				document.forms(0).Cell1.S(1,k+4,0,m+1);
				...
				document.forms(0).Cell1.S(8,k+4,0,orgId[k]);
				document.forms(0).Cell1.s(9,k+4,0,cardNum[k]);
				document.forms(0).Cell1.S(11,k+4,0,houseAddr[k]);
				document.forms(0).Cell1.S(14,k+4,0,houseTotalPrice[k]);
				
				document.forms(0).Cell1.S(16,k+4,0,loanMoney[k]);
				document.forms(0).Cell1.s(17,k+4,0,loanTime[k]);
				document.forms(0).Cell1.S(18,k+4,0,loanCity[k]);
				document.forms(0).Cell1.S(19,k+4,0,beiYong[k]);
				if(m!=0){
					tempId=id[k-1];
					if(tempId==id[k]){
						document.forms(0).Cell1.MergeCells(1,k+3,1,k+4);
					}else{
						m++;
					}
				}else{
				...
				document.forms(0).Cell1.S(1,k-completeline+2,pageCurrent,m+1);
				...
				document.forms(0).Cell1.S(8,k-completeline+2,pageCurrent,orgId[k]);
				document.forms(0).Cell1.s(9,k-completeline+2,pageCurrent,cardNum[k]);
				document.forms(0).Cell1.S(11,k-completeline+2,pageCurrent,houseAddr[k]);
				document.forms(0).Cell1.S(14,k-completeline+2,pageCurrent,houseTotalPrice[k]);
				document.forms(0).Cell1.S(16,k-completeline+2,pageCurrent,loanMoney[k]);
				document.forms(0).Cell1.s(17,k-completeline+2,pageCurrent,loanTime[k]);
				document.forms(0).Cell1.S(18,k-completeline+2,pageCurrent,loanCity[k]);
				document.forms(0).Cell1.S(19,k-completeline+2,pageCurrent,beiYong[k]);
		...
		document.forms(0).Cell1.S(16,totalLine+4,pageCurrent,"操作员");
		document.forms(0).Cell1.S(17,totalLine+4,pageCurrent,"<%=optername%>");
		document.forms(0).Cell1.S(18,totalLine+4,pageCurrent,"操作时间");
		document.forms(0).Cell1.S(19,totalLine+4,pageCurrent,"<%=time%>");
