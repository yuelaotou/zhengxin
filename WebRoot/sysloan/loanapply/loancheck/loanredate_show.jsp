...
function loads(){
	...
	for(i=0;i<document.all.length;i++){
		if(document.all.item(i).type=="submit"){
			if(document.all.item(i).value=="回件确认"){
				s1=i;
			}
			if(document.all.item(i).value=="撤销回件确认"){
				s2=i;
			}
			if(document.all.item(i).value=="列表打印"){
				s3=i;
			}
			if(document.all.item(i).value=="扫描他项权利证"){
				s4=i;
			}
		}
	}
...
function update(){
	// 回件日期
	var date=document.getElementById(tr).childNodes[14].innerHTML.trim();
	var status=document.getElementById(tr).childNodes[13].innerHTML.trim();
	if(status=="审批通过" || status=="拨款审核未通过"){
							...
							<table width="255" border="0" cellspacing="0" cellpadding="0"
								...
								<tr>
									...
									<td width="169" class=font14 bgcolor="#FFFFFF" align="center"
										valign="middle">
										<font color="00B5DB">申请贷款&gt;回件确认</font>
									</td>
				...
				<table width="98%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td valign="top" class=td3>
							...
							<html:form action="/loanRedateMaintainAC.do" method="post"
								style="margin: 0">
									...
									<logic:notEmpty name="loancheckShowAF" property="list">
										...
										<logic:iterate name="loancheckShowAF" property="list"
											...
											<tr id="tr<%=i%>" onclick='gotoClickpp("<%=i%>",idAF);gettr("tr<%=i%>");'
												...
												<td>
													<bean:write name="element" property="reDate" />
												</td>
												<td>
													<bean:write name="element" property="vipcheckDate" />
												</td>
												<td>
													<a
										...
										<td colspan="7" bgcolor="#FFFFFF" align="center" height="30">
											<table border="0" cellspacing="0" cellpadding="0">
												<tr>
													<td width="70">
														<html:submit property="method" styleClass="buttona">
															<bean:message key="button.redate.sure" />
														</html:submit>
													</td>
													<td width="90">
														<html:submit property="method" styleClass="buttonb">
															<bean:message key="button.redate.sure.del" />
														</html:submit>
													</td>
													<td width="70">
														<html:submit property="method" styleClass="buttona">
													...
													<td width="70">
														<html:submit property="method" styleClass="buttonc">
															<bean:message key="button.scan.others" />
														</html:submit>
													</td>
												</tr>
											</table>
