...
<script src="<%=path%>/js/common.js"></script>
  ...
  <script type="text/javascript">
  	function load(){
  		loginReg();
	  	document.forms(0).Cell1.openfile("<%=path%>/sysloan/accounthandle/report/adjustaccount.cll","");
	    <%
    	    LoancallbackTaAF af =(LoancallbackTaAF)request.getAttribute("LoancallbackTaAF");
    	    String docNum=af.getDocNum();
    	    String noteNum="";
    	    if(af.getNoteNum()!=null){
    	    	noteNum=af.getNoteNum();
    	    }
	...
	%>
		...
		if(sumPayC<0){
			...
			if(count!=-1){//计算小数后面
		     ...
		     document.forms(0).Cell1.S(14,4,0,noteNum);
		...
		if(sumPayI<0){
			...
			if(countI!=-1){//计算小数后面
		     ...
		     document.forms(0).Cell1.S(8,23,0,docNum);
		     document.forms(0).Cell1.S(14,23,0,"<%=noteNum%>");
